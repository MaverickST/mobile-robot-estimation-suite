...
/* Constants */
#define MIN_VALUE 1e-6f
#define CONV_MAX_KERNEL_FFT_INSTEAD 50
...
typedef enum {
    NORM_METHOD_1,
    NORM_METHOD_L1 = NORM_METHOD_1,
    NORM_METHOD_2,
    NORM_METHOD_L2 = NORM_METHOD_2,
    NORM_METHOD_INF,
    NORM_METHOD_FROBENIUS
} NORM_METHOD;

/* Helper functions */
static inline float vmax(float a, float b) {
    return (a > b) ? a : b;
}

static inline float vmin(float a, float b) {
    return (a < b) ? a : b;
}

static inline bool issymmetric(const float A[], size_t row, size_t column) {
    if (row != column) return false;
    for (size_t i = 0; i < row; i++) {
        for (size_t j = i + 1; j < column; j++) {
            if (fabsf(A[i * column + j] - A[j * column + i]) > MIN_VALUE) {
                return false;
            }
        }
    }
